...
//  public Vector4 pos = new Vector4(0,0,0);
    ...
    public Vector3 Mult(Vector4 position, Vector4 delta)
    {
        ...
        float y = ((identity[0, 0] * position.y) + (identity[0, 1] * position.y) + (identity[0, 2] * position.y) + (identity[0, 3] * position.y));
        float z = ((identity[0, 0] * position.z) + (identity[0, 1] * position.z) + (identity[0, 2] * position.z) + (identity[0, 3] * position.z));
